...
#endif
{
	...
	if(ix<0x3fd00000) return __kernel_sin(gM(pi,x),zero,0);
	...
	switch (n) {
	    case 0:   y =  __kernel_sin(gM(pi, y),zero,0); break;
	    case 1:   
	    case 2:   y =  __kernel_cos(gM(pi, gS(0.5,y)),zero); break;
	    case 3:  
	    case 4:   y =  __kernel_sin(gM(pi, gS(one,y)),zero,0); break;
	    case 5:
	    case 6:   y = -__kernel_cos(gM(pi, gS(y,1.5)),zero); break;
	    default:  y =  __kernel_sin(gM(pi, gS(y,2.0)),zero,0); break;
	    }
	return -y;
}
...
#endif
{
	...
	*signgamp = 1;
	...
	if(hx<0) {
	    ...
	    nadj = __ieee754_log(gD(pi, fd_fabs(gM(t, x))));
